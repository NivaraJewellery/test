...
function buildOrderSummary(order, normalizedOrder, paymentId, orderId) {
  ...
  const subtotal = promo?.subtotal != null ? Number(promo.subtotal) : products.reduce((sum, item) => sum + Number(item.price || 0) * Number(item.quantity || 1), 0);
  return {
    orderId, paymentId, amount: Number(order.amount || 0), subtotal, promo,
    customer: normalizedOrder.customer || {}, products, createdAt: new Date().toISOString(), status: 'confirmed'
  };
}
...
module.exports = async function handler(request, response) {
  ...
  if (alreadyVerified) {
    return send(response, 200, { verified: true, alreadyVerified: true, paymentId: razorpay_payment_id, orderId: razorpay_order_id, order: buildOrderSummary(order, normalizedOrder, razorpay_payment_id, razorpay_order_id) });
  }
    ...
    return send(response, 200, {
      ...
      order: buildOrderSummary(order, normalizedOrder, razorpay_payment_id, razorpay_order_id)
    });
  } catch (error) {
