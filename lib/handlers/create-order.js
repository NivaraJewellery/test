  ...
  try {
    ...
    for (const item of items) {
      ...
      const rows = await sql`
        select id, name, type, category, price, stock, code, image
        from products
      ...
      if (variantRows.length) {
        ...
        const requestedUom = String(item.uom || '').trim();
        const identityMismatch = Boolean(variant && (
          (requestedSize && String(variant.size || '').trim() !== requestedSize) ||
          (requestedUom && String(variant.uom || '').trim() !== requestedUom)
        ));
        if (!variant || identityMismatch) {
          const label = [requestedSize, requestedUom].filter(Boolean).join(' ');
          return send(response, 400, { error: `${product.name}${label ? ` (${label})` : ''} is no longer available. Please choose an available size.` });
        }
    ...
    await sql.begin(async transaction => {
      ...
      await transaction`
        ...
        values (${order.id}, ${amount}, ${shippingCharge}, ${customerDetails.email || null}, ${JSON.stringify({
          ...
          products: orderItems.map(item => ({ id: item.product.id, name: item.product.name, type: item.product.type || '', category: item.product.category || '', price: item.product.price, quantity: item.quantity, image: item.product.image || '', variantId: item.variant?.id || null, size: item.variant?.size || '', uom: item.variant?.uom || '' }))
        })}, 'pending_payment')
      `;
    });
