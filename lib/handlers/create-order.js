...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    const profileAddress = selectedAddress ? formatAddress({ line1:selectedAddress.line1, line2:selectedAddress.line2, city:selectedAddress.city, state:selectedAddress.state, pincode:selectedAddress.pincode }) : formatAddress(body.customer?.address);
    ...
    const customerDetails = {
      name: String(selectedAddress?.recipient_name || body.customer?.name || guest.name || '').trim(),
      email: customerEmail,
      phone: String(selectedAddress?.phone || body.customer?.phone || guest.phone || '').trim(),
      shippingAddress,
      billingAddress,
    ...
    await sql.begin(async transaction => {
      ...
      await transaction`
        ...
        values (${order.id}, ${amount}, ${customerDetails.email || null}, ${JSON.stringify({
          ...
          products: orderItems.map(item => ({ id: item.product.id, name: item.product.name, price: item.product.price, quantity: item.quantity, image: item.product.image || '' }))
        })}, 'pending_payment')
      `;
    });
