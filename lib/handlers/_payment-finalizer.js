...
function normalizeOrderItems(items) {
  ...
  if (Array.isArray(parsed)) return { customer: {}, products: parsed, promo: null, shipping: null };
  return {
...
function buildOrderSummary(order, normalizedOrder, paymentId, orderId) {
  ...
  return {
    ...
    subtotal,
    promo,
    shippingCharge: Number(normalizedOrder.shipping?.charge || order.shipping_charge || 0),
...
async function loadOrderByRazorpayOrderId(sql, razorpayOrderId) {
  const rows = await sql`
    select id, razorpay_order_id, razorpay_payment_id, items, amount, shipping_charge, customer_email, status, created_at
    from orders
...
async function finalizePaidOrder(sql, order, razorpayPaymentId) {
  ...
  await sql.begin(async transaction => {
    ...
    for (const item of storedItems) {
      ...
      if (Number(item.variantId)) {
        const updatedVariants = await transaction`
          update product_variants set stock=stock-${quantity}, updated_at=now()
          where id=${Number(item.variantId)} and product_id=${productId} and active=true and stock>=${quantity}
          returning id, stock
        `;
        if (!updatedVariants.length) throw new Error(`${item.name || `Product ${productId}`} (${item.size || ''} ${item.uom || ''}) is out of stock or unavailable.`);
        await transaction`update products set stock=(select coalesce(sum(stock),0) from product_variants where product_id=${productId} and active=true), updated_at=now() where id=${productId}`;
      } else {
        const updatedProducts = await transaction`update products set stock=stock-${quantity}, updated_at=now() where id=${productId} and stock>=${quantity} returning id, stock`;
        if (!updatedProducts.length) throw new Error(`${item.name || `Product ${productId}`} is out of stock or unavailable.`);
      }
    }
...
async function sendOrderNotifications(order, normalizedOrder, paymentId) {
  ...
  const itemRows = storedItems.map(item => { const isBangle=String(item.type||item.category||'').toLowerCase().includes('bangle') || Boolean(item.variantId && item.size); return `<li>${item.name || `Product ${item.id}`}${isBangle&&item.size ? ` — Size: ${item.size}${item.uom ? ` ${item.uom}` : ''}` : ''} x ${item.quantity} - Rs. ${Number(item.price || 0).toLocaleString('en-IN')}</li>`; }).join('');
  const productNames = storedItems.length
    ? storedItems.map(item => { const isBangle=String(item.type||item.category||'').toLowerCase().includes('bangle') || Boolean(item.variantId && item.size); return `${item.name || `Product ${item.id}`}${isBangle&&item.size ? ` — Size: ${item.size}${item.uom ? ` ${item.uom}` : ''}` : ''} x ${item.quantity}`; }).join(', ')
  ...
  const shippingCharge = Number(normalizedOrder.shipping?.charge || order.shipping_charge || 0);
  const shippingText = shippingCharge === 0 ? 'FREE' : `Rs. ${shippingCharge.toLocaleString('en-IN')}`;
