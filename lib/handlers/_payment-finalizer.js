...
async function sendOrderNotifications(order, normalizedOrder, paymentId) {
  ...
  const itemRows = storedItems.map(item => `<li>${item.name || `Product ${item.id}`}${item.size ? ` (${item.size} ${item.uom})` : ''} x ${item.quantity} - Rs. ${Number(item.price || 0).toLocaleString('en-IN')}</li>`).join('');
  const productNames = storedItems.length
    ? storedItems.map(item => `${item.name || `Product ${item.id}`}${item.size ? ` (${item.size} ${item.uom})` : ''} x ${item.quantity}`).join(', ')
