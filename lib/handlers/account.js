...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (action === 'signup') {
      ...
      return send(response, 201, { otpRequired: true, pendingCustomer: { id: created[0].id, email: created[0].email, name: created[0].name } });
    }
    ...
    if (action === 'signup-verify') {
      ...
      const customers = await sql`
        select id, name, email, pending_phone from customers
        where id = ${customerId} and email = ${email} and phone_otp = ${otp} and phone_otp_expires_at > now()
        limit 1
      `;
      if (!customers.length) return send(response, 400, { error: 'OTP is invalid or expired' });
      await ensurePhoneAvailable(sql, normalizePhone(customers[0].pending_phone), customerId);
      await sql`
        update customers
        set phone = ${customers[0].pending_phone}, phone_verified = true, pending_phone = null, phone_otp = null, phone_otp_expires_at = null
        where id = ${customerId}
      `;
    ...
    if (action === 'resend-phone-otp') {
      ...
      await sendPhoneOtp(sql, customer, phone);
    ...
    if (action === 'profile-update') {
      ...
      const customer = await getCustomer(sql, current);
      if (!customer) return send(response, 404, { error: 'Customer not found' });
      const phoneChanged = phone !== normalizePhone(customer.phone);
      if (phoneChanged) {
        await ensurePhoneAvailable(sql, phone, current.id);
        await sql`update customers set name = ${name} where id = ${current.id}`;
        await sendPhoneOtp(sql, { ...customer, name }, phone);
        const updated = await getCustomer(sql, current);
        return send(response, 200, { customer: publicCustomer(updated), otpRequired: true, pendingPhone: phone });
      }
      await sql`update customers set name = ${name} where id = ${current.id}`;
      const updated = await getCustomer(sql, current);
    ...
    if (action === 'verify-phone') {
      ...
      const customers = await sql`
        select id, pending_phone from customers
        where id = ${current.id} and email = ${current.email} and phone_otp = ${otp} and phone_otp_expires_at > now()
        limit 1
      `;
      if (!customers.length) return send(response, 400, { error: 'OTP is invalid or expired' });
      await ensurePhoneAvailable(sql, normalizePhone(customers[0].pending_phone), current.id);
      await sql`update customers set phone = ${customers[0].pending_phone}, phone_verified = true, pending_phone = null, phone_otp = null, phone_otp_expires_at = null where id = ${current.id}`;
      const updated = await getCustomer(sql, current);
      return send(response, 200, { customer: publicCustomer(updated) });
    }
